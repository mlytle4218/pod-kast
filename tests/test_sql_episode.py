...
class TestSqlCategoryClass:
    ...
    def test_add_new_episode(self):
        ...
        result = da.add_new_episode(self.session,test3)
        assert result.title == test3.title

        result = da.add_new_episode(self.session,test4)
        assert result.title == test4.title

        result = da.add_new_episode(self.mock,test1)
        assert result == None

    def test_get_all_episodes(self):
        result = da.get_all_episodes(self.session)
        assert len(result) == 4

        result = da.get_all_episodes(self.mock)
        assert result == None

    def test_get_episode_by_id(self):
        result = da.get_episode_by_id(self.session, 1)
        assert result.title == test1.title

        result = da.get_episode_by_id(self.mock, 1)
        assert result == None

    def test_get_episodes_by_podcast_id(self):
        result = da.get_episodes_by_podcast_id(self.session, 1)
        assert len(result) == 2

        result = da.get_episodes_by_podcast_id(self.mock, 1)
        assert result == None

    def test_get_episodes_with_downloads_available_by_podcast_id(self):
        result = da.get_episodes_with_downloads_available_by_podcast_id(self.session, 1)
        assert len(result) == 2

        result = da.get_episodes_with_downloads_available_by_podcast_id(self.mock, 1)
        assert result == None
